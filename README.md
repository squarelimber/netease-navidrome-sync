...
- **网易云手机扫码登录**：无需手动抓 Cookie，状态页点"显示二维码"用 App 扫码即可自动注入登录态
- **轻量状态页**：浏览器查看 Cookie 健康、统计、运行历史、失败队列，可暂停自动刷新、手动触发运行/中止、逐条重试失败曲目
...
4. 复制一份配置模板并编辑：
   ...
   ```
5. 配置网易云 Cookie（二选一）：
   - **推荐**：先启动服务（第 7 步），在状态页点"显示二维码"用网易云 App 扫码登录
   - **手动**：提前准备 Cookie 写入 `data/cookie.txt`（方法见后文）
...
1. 启动服务后打开状态页 `http://你的IP:8678`
2. 点击卡片中的 **"显示二维码"**
3. 用 **网易云音乐 App** 扫码
4. 在手机上点确认登录——页面出现 **"✓ 登录成功，Cookie 已更新"**
5. Cookie 会自动写入配置文件并生效，无需任何手动操作
...
- 网易云 Cookie 是否有效
- 下次运行时间
- 各曲目状态统计（已下载 / 已存在 / 失败 / 放弃 / 待处理）
- 最近运行记录（下载 / 跳过 / 失败数量 + 耗时，不再显示原始 JSON）
- 最近入库列表（可滚动）
- 失败/重试队列（可滚动），含失败原因与下次重试时间，可逐条点击"重试"
- **停止按钮**：运行中的任务可在下一首曲目前中止
- **暂停自动刷新**：暂停或恢复 30 秒自动轮询
- **网易云扫码登录**：点"显示二维码"生成 QR 码，App 扫码后状态实时更新"等待扫码→已扫码待确认→登录成功"，Cookie 自动注入并持久化
